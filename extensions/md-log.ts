 ...
 *
 * Commands:
 *   /md-log             — Create/link a session-named note in Obsidian, backfill, and open it.
 *   /md-log <filepath>  — Link an existing markdown file, backfill, and open it in Obsidian.
...
const DEFAULT_LOG_DIR = path.join(process.env.HOME || "", "Documents/Pi agent/PiLog");
...
function safeMarkdownFilename(name: string): string {
	const sanitized = name
		...
		.replace(/^[. ]+|[. ]+$/g, "")
		.slice(0, 120);
	const base = sanitized || `Session ${timestampForFilename()}`;
...
export default function mdLog(pi: ExtensionAPI) {
	...
	/**
	 * Keep the session/TUI source untouched, but serialize LaTeX delimiters in
	 * the Markdown form that Obsidian's renderer expects. Inline/code-fenced
	 * code is copied verbatim so syntax examples remain literal examples.
	 */
	function toObsidianMathMarkdown(text: string): string {
		const output: string[] = [];
		let index = 0;

		while (index < text.length) {
			// Preserve inline code and fenced code blocks, including their contents.
			if (text[index] === "`") {
				let tickEnd = index;
				while (text[tickEnd] === "`") tickEnd++;
				const ticks = text.slice(index, tickEnd);
				const close = text.indexOf(ticks, tickEnd);
				if (close === -1) {
					output.push(ticks);
					index = tickEnd;
					continue;
				}
				output.push(text.slice(index, close + ticks.length));
				index = close + ticks.length;
				continue;
			}

			// LaTex's \[...\] is not reliable in Obsidian Markdown. Use a
			// dedicated MathJax block instead, retaining the formula itself.
			if (text.startsWith("\\[", index)) {
				const close = text.indexOf("\\]", index + 2);
				if (close !== -1) {
					const formula = text.slice(index + 2, close).trim();
					output.push(`$$\n${formula}\n$$`);
					index = close + 2;
					continue;
				}
			...
			// Likewise, write inline math as $...$ for Obsidian.
			if (text.startsWith("\\(", index)) {
				const close = text.indexOf("\\)", index + 2);
				if (close !== -1) {
					output.push(`$${text.slice(index + 2, close)}$`);
					index = close + 2;
					continue;
				}
			}
	...
	function callout(type: string, title: string, bodyLines: string[]): string {
		...
		for (const line of toObsidianMathMarkdown(bodyLines.join("\n")).split("\n")) {
	...
	function userBlock(text: string): string {
		// Preserve the user's original terminal input exactly in the log.
		return `> [!quote] YOU\n\n${text}`;
	}
	...
	pi.registerCommand("md-log", {
		description: "Create/link a session note, backfill history, and open it in Obsidian",
		handler: async (args, ctx: any) => {
			...
			let resolved: string;
			if (filepath) {
				resolved = path.isAbsolute(filepath) ? filepath : path.resolve(ctx.cwd, filepath);
				...
				const sessionName = pi.getSessionName()?.trim() || `Session ${timestampForFilename()}`;
				fs.mkdirSync(DEFAULT_LOG_DIR, { recursive: true });
				resolved = path.join(DEFAULT_LOG_DIR, safeMarkdownFilename(sessionName));
			...
			const opened = await pi.exec("open", ["-a", "Obsidian", resolved]);
			if (opened.code !== 0) {
				ctx.ui.notify(`Linked, but Obsidian could not open the note: ${opened.stderr || resolved}`, "warning");
			}
		},
	});
