...
function addMarkdown(
	...
	const contentWidth = Math.max(1, width - visibleWidth(indent));
	for (const line of renderMarkdown(markdown, contentWidth, color)) {
		lines.push(truncateToWidth(`${indent}${line}`, width));
	}
}

function addMarkdownWithPrefix(
	lines: string[],
	markdown: string,
	width: number,
	firstPrefix: string,
	...
	const rendered = renderMarkdown(markdown, Math.max(1, width - prefixWidth), color);
	for (let i = 0; i < rendered.length; i++) {
		const prefix = i === 0 ? firstPrefix : continuationPrefix;
		lines.push(truncateToWidth(`${prefix}${rendered[i]}`, width));
	}
}
...
function renderFeedback(
	...
	for (let i = 0; i < options.length; i++) {
		...
		const firstPrefix = theme.fg(color, ` ${marker} ${index}. `);
		addMarkdownWithPrefix(
			lines,
			opt.label,
			width,
			firstPrefix,
			" ".repeat(visibleWidth(firstPrefix)),
	...
	if (dontKnow) {
		...
		const correctStr = correctIndices.map((i) => formatOptionRef(options, i)).join(", ");
		addMarkdown(lines, `Correct answer: ${correctStr}`, width, " ", (text) => theme.fg("muted", text));
	} else if (correct) {
	...
	if (explanation) {
		lines.push("");
		addMarkdown(lines, explanation, width, " ", (text) => theme.fg("text", text));
	}
	lines.push("");
...
function pushHeader(lines: string[], theme: any, width: number, question: string, context: string | undefined): void {
	...
	addMarkdown(lines, question, width, " ", (text) => theme.fg("text", text));
	if (context) {
		lines.push("");
		addMarkdown(lines, context, width, " ", (text) => theme.fg("muted", text));
	}
}
		...
		(tui: any, theme: any, _kb: any, done: (result: QuizResponse | null) => void) => {
			...
			function render(width: number): string[] {
				...
				for (let i = 0; i < allOptions.length; i++) {
					...
					const numberPrefix = theme.fg(selected ? "accent" : "text", `${option.index}. `);
					const firstPrefix = `${cursorPrefix}${numberPrefix}`;
					addMarkdownWithPrefix(
						lines,
						option.label,
						width,
						firstPrefix,
						" ".repeat(visibleWidth(firstPrefix)),
						(text) => theme.fg(selected ? "accent" : "text", text),
					);
					if (option.description) {
						addMarkdown(lines, option.description, width, "     ", (text) => theme.fg("muted", text));
					}
				}
		...
		(tui: any, theme: any, _kb: any, done: (result: QuizResponse | null) => void) => {
			...
			function render(width: number): string[] {
				...
				for (let i = 0; i < allItems.length; i++) {
					...
					const markerPrefix = theme.fg(color, `${marker} ${item.index}. `);
					const firstPrefix = `${prefix}${markerPrefix}`;
					addMarkdownWithPrefix(
						lines,
						item.label,
						width,
					...
					if (item.description) {
						addMarkdown(lines, item.description, width, "     ", (text) => theme.fg("muted", text));
					}
				}
...
export default function quiz(pi: ExtensionAPI) {
	pi.registerTool({
		...
		label: "quiz",
		description:
			"Ask the user a graded multiple-choice or multi-select question with instant scoring and feedback. Options only; an 'I don't know' choice is automatically added. For non-graded input or preferences, use ask_user_question.",
		promptSnippet:
			"Test the user with a graded multiple-choice or multi-select question (required correct answer + explanation).",
		promptGuidelines: [
			"quiz is GRADED; ask_user_question is not. Use quiz only when there is a determinate correct answer.",
			'correctAnswer is REQUIRED and must match the option `value` (single-select: string; multi-select: string[]).',
			"explanation is REQUIRED — briefly explain why the correct answer is correct (revealed after answering).",
			"Do not add an 'I don't know' option manually; it is added automatically.",
			"For distractor construction and pedagogical probing rules, follow the active teach or feynman-check skill.",
		],
