...
function markdownToPlainText(markdown: string): string {
	return renderMarkdown(markdown, 4096)
	...
	return ctx.ui.custom<AskAnswer | null>((tui: any, theme: any, _kb: any, done: (result: AskAnswer | null) => void) => {
		...
		function render(width: number): string[] {
			...
			for (let i = 0; i < allOptions.length; i++) {
				...
				const numberPrefix = option.isOther
					? ""
					: selected
						? theme.fg("accent", `${option.index}. `)
						: theme.fg("text", `${option.index}. `);
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			...
			return withUILock(async () => {
				if (mode === "text") {
					const editorMarkdown = context ? `${params.question}\n\n${context}` : params.question;
					const answer = await ctx.ui.editor(markdownToPlainText(editorMarkdown));
					if (answer === undefined) {
						return cancelledResult(params.question, mode, context);
					}
